...
import React, { useContext, useState, forwardRef, Dispatch } from 'react';
...
export const HeaderFilters = forwardRef<HTMLUListElement, HeaderFiltersProps>(({activeDropdown, setActiveDropdown}, ref) => {
  const filter = useContext(FilterContext);

  const size: IUseWindowSize = useWindowSize();

  return (
    <ul ref={ref} className={size.width > 768 ? 'flex flex-col w-full md:w-auto md:flex-row items-center md:space-x-[8px]' : 'flex flex-col justify-start items-center h-full space-y-[20px]'}>
      <Dropdown
        filterName="Category"
        fields={categories}
